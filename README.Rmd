...
especially when making reports. Included are some functions for quickly setting
up HTML notebooks with `{rmarkdown}` and `{bookdown}`, and some helpful
...
and a suitable use of `base::list.files()`.

## Plans for updates
* Add `use_rmd_pdf()`, `use_rmd_powerpoint()` and friends to automate these
  tasks as well.
